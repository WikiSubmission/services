...
export default function listener(): void {
  PrivateBot.shared.addEventListener("guildMemberAdd", async (member) => {
    ...
    if (
      ...
      if (memberRecord.data) {
        ...
        if (
          ...
          await resolvedMember.jail(PrivateBot.shared.client.user, "Re-joined");
          await new DiscordAlert(resolvedMember.moderatedGuild).send(
            "JAIL-CHAT",
            {
              content: `...you're back.`,
              ...
              embeds: [
                new EmbedBuilder()
                  ...
                  .addFields(
                    ...
                    {
                      name: "Alert",
                      value: `Brand new account – created **${DateUtils.distanceFromNow(member.user.createdTimestamp)}**`,
                    },
                  )
      ...
      async function updateMemberDatabaseRecord() {
        ...
        if (request.error) {
          PrivateBot.shared.logError(request.error);
        }
      }
    }
  });
